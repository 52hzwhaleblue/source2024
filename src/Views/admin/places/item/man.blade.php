...
<div class="container-xxl flex-grow-1 container-p-y container-fluid">
    ...
    <div class="card pd-15 bg-main mb-3">
        <div class="row">
            ...
            <div class="form-group col-md-3">
                {!! Func::getLinkCategory('district', 'district', $type, 'Quận/Huyện') !!}
            </div>
        </div>
    </div>
            ...
            <table class="datatables-category-list table border-top text-sm">
                ...
                @forelse($items as $k => $v)
                    ...
                    <td class="align-middle w-[70px] !pl-0">
                        @component('component.inputNumb',['numb'=>$v['numb'],'idtbl'=>$v['id'],'table'=>'news_item'])@endcomponent
                    </td>
                    <td class="align-middle">
                        <a class="text-dark text-break">{{ $v['namevi'] }}</a>
                        <div class="tool-action mt-2 w-clear">
                            @component('component.buttonAction',['config'=>$configMan->categories->item,'slug'=>$v['slugvi'],'params'=>['id'=>$v['id'],'id_city'=>$v['id_city'],'id_district'=>$v['id_district']]])
                                @if (!empty($configMan->categories->item->copy_categories))
                                    <div class="dropdown">
                                        <a id="dropdownCopy" data-url="{{ url('copy') }}" data-id="{{ $v['id'] }}" data-table="news_item" data-com="{{ $com }}" data-type="{{ $type }}" class="nav-link text-success mr-3"><i class="ti ti-copy"></i>Copy</a>
                                    </div>
                                @endif
                            @endcomponent
                        </div>
                        ...
                        @foreach ($configMan->categories->item->status_categories as $key => $value)
                            ...
                            <td class="align-middle text-center">
                                <label class="switch switch-success">
                                    @component('component.switchButton',['keyC'=>$key,'idC'=>$v['id'],'tableC'=>'news_item','status_arrayC'=>$status_array]) @endcomponent
                                </label>
                            </td>
                        @endforeach
                    @endif
                    <td class="align-middle text-center">
                        @component('component.buttonList',['params'=>['id'=>$v['id'],'id_city'=>$v['id_city'],'id_district'=>$v['id_district']]])@endcomponent
                    </td>
                </tr>
                @empty
                    <tr>
