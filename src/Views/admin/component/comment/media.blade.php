<div class="carousel slide carousel-comment-media mt-2" id="carousel-comment-media-{{ $params['id'] }}" data-bs-ride="carousel" data-bs-touch="true" data-interval="false">
    <ol class="carousel-indicators w-clear">
        ...
        <li class="position-relative transition me-1 float-left">
            <a data-fancybox="gallery" data-src="{{ upload('file', $params['video']) }}">
                <img class="w-100" onerror="this.src='assets/images/noimage.png';" src="{{ assets_photo('photo', '70x70x1', $params['photo'], 'thumbs') }}" alt="{{ $params['name'] }}" title="{{ $params['name'] }}" />
      ...
      C97.3,23.7,75.7,2.3,49.9,2.5" />
        ...
        <li class="transition me-1 float-left">
            <a data-fancybox="gallery" data-src="{{ upload('photo', $v_photo['photo']) }}" title="{{ $params['name'] }}">
                <img class="w-100" onerror="this.src='assets/images/noimage.png';" src="{{ assets_photo('photo', '70x70x1', $v_photo['photo'], 'thumbs') }}" alt="{{ $params['name'] }}" title="{{ $params['name'] }}" />
            </a>
        </li>
        @endforeach
        @endif
