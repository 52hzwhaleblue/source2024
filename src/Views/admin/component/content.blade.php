    ...
    <div class="tab-content" id="custom-tabs-three-tabContent-lang">
        @foreach (config('app.langs') as $k => $v)
            <div class="tab-pane fade show {{ $k == 'vi' ? 'active' : '' }}" id="tabs-lang-{{ $k }}"
                ...
                @if (!empty($name1))
                    <div class="form-group last:!mb-0">
                        <label class="form-label" for="name1{{ $k }}">Tiêu đề 1 ({{ $k }}) :</label>
                        <input type="text" class="form-control text-sm" name="data[name1{{ $k }}]"
                            id="name1{{ $k }}" placeholder="Tiêu đề 1 ({{ $k }})"
                            value="{{ !empty(Flash::has('name1vi')) ? Flash::get('name1vi') : $item['name1' . $k] }}"
                            >
                    </div>
                @endif
