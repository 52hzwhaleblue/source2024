...
<div class="card pd-15 bg-main mb-3 navbar-detached">
    <div class="d-flex gap-2">
        @if (Func::chekcPermission($tb . $kind . '.' . $type . '.add', $permissions))
            @if ($com != 'order')
                <a class="btn btn-primary text-white {{ !isPermissions(str_replace('-', '.', $com) . '.' . $type . '.add') ? 'disabled' : '' }}"
        ...
        @if (Func::chekcPermission($tb . $kind . '.' . $type . '.delete', $permissions))
            <a class="btn btn-secondary text-white {{ !isPermissions(str_replace('-', '.', $com) . '.' . $type . '.delete') ? 'disabled' : '' }}"
                id="delete-all"
