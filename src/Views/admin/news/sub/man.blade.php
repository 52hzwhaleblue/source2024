                ...
                @forelse($items as $k => $v)
                    <tr>
                        ...
                        <td class="align-middle w-[70px] !pl-0">
                            @component('component.inputNumb',['numb'=>$v['numb'],'idtbl'=>$v['id'],'table'=>'news_sub'])@endcomponent
                        </td>
                        <td class="align-middle">
                            ...
                            <div class="tool-action mt-2 w-clear">
                                @component('component.buttonAction',['config'=>$configMan->categories->sub,'slug'=>$v['slugvi'],'params'=>['id'=>$v['id'],'id_list'=>$v['id_list'],'id_cat'=>$v['id_cat'],'id_item'=>$v['id_item']]])
                                    @if (!empty($configMan->categories->sub->copy_categories))
                                        <div class="dropdown">
                                            <a id="dropdownCopy" data-url="{{ url('copy') }}" data-id="{{ $v['id'] }}" data-table="news_sub" data-com="{{ $com }}" data-type="{{ $type }}" class="nav-link text-success mr-3"><i class="ti ti-copy"></i>Copy</a>
                                        </div>
                                    @endif
                                @endcomponent
                            </div>
                        ...
                        @if (!empty($configMan->categories->sub->show_images_categories))
                            <td class="align-middle">
                                <img class="img-preview" onerror="this.src='../assets/images/noimage.png';" src="{{assets_photo('news','70x70x1',$v['photo'],'thumbs')}}"  alt="{{ $v['namevi'] }}" title="{{ $v['namevi'] }}" />
                            </td>
                        @endif
                        @if (!empty($configMan->categories->sub->status_categories))
                            @foreach ($configMan->categories->sub->status_categories as $key => $value)
                                ...
                                <td class="align-middle text-center">
                                    <label class="switch switch-success">
                                        @component('component.switchButton',['keyC'=>$key,'idC'=>$v['id'],'tableC'=>'news_sub','status_arrayC'=>$status_array]) @endcomponent
                                    </label>
                                </td>
                            @endforeach
                        @endif
