                ...
                @forelse($items as $k => $v)
                    <tr>
                        ...
                        <td class="align-middle w-[70px] !pl-0">
                            @component('component.inputNumb',['numb'=>$v['numb'],'idtbl'=>$v['id'],'table'=>'news_cat'])@endcomponent
                        </td>
                        <td class="align-middle">
                            ...
                            <div class="tool-action mt-2 w-clear">
                                @component('component.buttonAction',['config'=>$configMan->categories->cat,'slug'=>$v['slugvi'],'params'=>['id'=>$v['id'],'id_list'=>$v['id_list']]])
                                    @if (!empty($configMan->categories->cat->copy_categories))
                                        <div class="dropdown">
                                            <a id="dropdownCopy" data-url="{{ url('copy') }}" data-id="{{ $v['id'] }}" data-table="news_cat" data-com="{{ $com }}" data-type="{{ $type }}" class="nav-link text-success mr-3"><i class="ti ti-copy"></i>Copy</a>
                                        </div>
                                    @endif
                                @endcomponent
                            </div>
                            ...
                            @foreach ($configMan->categories->cat->status_categories as $key => $value)
                                ...
                                <td class="align-middle text-center">
                                    <label class="switch switch-success">
                                        @component('component.switchButton',['keyC'=>$key,'idC'=>$v['id'],'tableC'=>'news_cat','status_arrayC'=>$status_array]) @endcomponent
                                    </label>
                                </td>
                            @endforeach
                        @endif
