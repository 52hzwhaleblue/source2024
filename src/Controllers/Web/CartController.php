...
class CartController extends Controller
{
    ...
    protected function showPrice($request): void
    {
        ...
        $row = ProductPropertiesModel::select(['code','regular_price', 'sale_price'])->where('id_parent', $id_product)->where('id_properties', $id_properties)->first();
        response()->json([
            'code' => $row['code'],
            'priceNew' => Func::formatMoney($row['sale_price']), 
            'priceOld' => Func::formatMoney($row['regular_price'])]);
    }
