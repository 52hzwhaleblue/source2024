...
class SettingController
{
    ...
    {
        $items = \NINA\Models\PhotoModel::select(['link', 'link_redirect', 'numb', 'id', 'redirect'])->where('type', $type)->orderBy('id', 'desc')->paginate(10);
        $count = \NINA\Models\PhotoModel::where('type', $type)->count();
        return view('setting.redirect.index', ['items' => $items ?? collect(), 'count' => $count]);
    }
    public function edit($com, $act, $type, Request $request)
    {
        $id = $request->query('id');
        $item = \NINA\Models\PhotoModel::find($id) ?? collect();
    ...
    protected function saveRedirect($com, $act, $type, $request)
    {
        ...
        if (!empty($id)) \NINA\Models\PhotoModel::where('id', $id)->update($data);
        else \NINA\Models\PhotoModel::create($data);
    ...
    public function delete($com, $act, $type, Request $request)
    {
        if (!empty($request->id)) {
            $id = $request->id;
            \NINA\Models\PhotoModel::where('id', $id)->delete();
