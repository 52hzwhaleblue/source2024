...
if (typeof $ !== 'undefined') {
	$(function () {
		...
		if (searchInput.length) {
			...
			var filterConfig = function (data) {
				...
				return function findMatches(q, cb) {
					...
					data.filter(function (i) {
						if (i.namevi.toLowerCase().startsWith(q.toLowerCase()) || i.slugvi.toLowerCase().startsWith(q.toLowerCase())) {
							matches.push(i);
						} else if (
							(!i.namevi.toLowerCase().startsWith(q.toLowerCase()) &&
							i.namevi.toLowerCase().includes(q.toLowerCase())) || (!i.slugvi.toLowerCase().startsWith(q.toLowerCase()) &&
							i.slugvi.toLowerCase().includes(q.toLowerCase()))
						) {
							matches.push(i);
							matches.sort(function (a, b) {
								return b.namevi < a.namevi ? 1 : -1;
							});
						} else {
							return [];
						}
			...
			// Search JSON
			var searchJson = 'slug.json'; // For vertical layout
			...
			var searchData = $.ajax({
				...
				//url: 'checkslug', //? Use your own search api instead
				dataType: 'json',
				async: false
			}).responseJSON;
				...
				searchInput
					.typeahead(
						...
						// Pages
						{
							...
							display: 'name',
							limit: 5,
							source: filterConfig(searchData.slug),

							templates: {
								header: '<h6 class="suggestions-header text-primary mb-0 mx-3 mt-3 pb-2">Trang</h6>',
								suggestion: function ({ com, type, namevi, slugvi }) {
									var link = com + '/man/' + type + '?search=' + com + '&keyword=' + namevi;
									return (
										'<a href="' +
										link +
										'">' +
										...
										namevi +
										'<i class="view-search-nav"> ( Xem chi tiết >> ) </i></span>' +
										'</div>' +
										'</a>'
									);
								},
...
if (comValue) {
	var layoutMenuActive = document.getElementsByClassName('menu-link');
