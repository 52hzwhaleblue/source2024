$.fn.comments = function(options){
    ...
    wrapper.on('click', '.btn-reply-comment', function(e){
        ...
        $this.text($this.text() == LANG['traloi'] ? LANG['huybo'] : LANG['traloi']);
    ...
    wrapper.on('click', '.btn-status-comment', function(e){
        ...
        $.ajax({
            ...
            success: function(response){
                if(response.errors)
                {
                    showNotify(response.errors, LANG['thongbao'], 'danger');
                }
                else
                {
                    showNotify(LANG['capnhattrangthaithanhcong'], LANG['thongbao'], 'success');
                    $this.parents(".comment-action").prevAll("." + newSibling).find(".comment-new").remove();
                    $this.text($this.text() == LANG['duyet'] ? LANG['boduyet'] : LANG['duyet']);
    ...
    wrapper.on('click', '.btn-delete-comment', function(e){
        ...
        $.confirm({
            ...
            content: LANG['banmuonxoabinhluannay'], // html, text
                ...
                success: {
                    ...
                    action: function(){
                        $.ajax({
                            ...
                            error: function(e){
                                holdonClose();
                                showNotify(LANG['hethongbiloivuilongthulaisau'], LANG['thongbao'], 'error');
                            },
                            success: function(response){
                                holdonClose();
                                ...
                                else
                                {
                                    ...
                                    showNotify(LANG['xoabinhluanthanhcong'], LANG['thongbao'], 'success');
    ...
    wrapper.on('click', '.btn-cancel-reply', function(e){
        ...
        $parents.find(".btn-reply-comment").text(LANG['traloi']);
    });
    ...
    wrapper.on('click', '.btn-load-more-comment-parent', function(e){
        ...
        $.ajax({
            ...
            beforeSend: function(){
                $this.text(LANG['dangtai']+"...");
                $this.attr("disabled", true);
            },
            error: function(e){
                $this.text(LANG['taithembinhluan']);
    ...
    wrapper.on('click', '.btn-load-more-comment-child', function(e){
        ...
        $.ajax({
            ...
            error: function(e){
                $this.text(LANG['xemthembinhluan']);
                $this.attr("disabled", false);
                showNotify(LANG['hethongbiloivuilongthulaisau'], LANG['thongbao'], 'error');
            },
            success: function(response){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    else
                    {
                        ...
                        holdonClose();
                        showNotify(LANG['phanhoithanhcong'], LANG['thongbao'], 'success');
                    }
                }
            });
        }, 500);
